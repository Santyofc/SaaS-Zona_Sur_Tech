...
import styles from "./RubeGoldbergForm.module.css";

export default function RubeGoldbergForm() {
    ...
    useEffect(() => {
        ...
        const initAnimation = () => {
            ...
            const containerEl = wrapper.querySelector('.container') as HTMLElement;
            const checkboxEl = wrapper.querySelector('.form-container .form-row input[type="checkbox"]') as HTMLInputElement;
            const nameEl = wrapper.querySelector('.form-container .form-row input[name="name"]') as HTMLInputElement;
            const emailEl = wrapper.querySelector('.form-container .form-row input[name="email"]') as HTMLInputElement;
            const submitBtn = wrapper.querySelector('.form-container .form-row input[type="submit"]') as HTMLInputElement;
            
            const sprayer = wrapper.querySelector('.sprayer') as HTMLElement;
            const sprayHandContainer = wrapper.querySelector('.spray-hand-container') as HTMLElement;
            const sprayLines = Array.from(wrapper.querySelectorAll('.spray-line'));
            const sprayBubbles = Array.from(wrapper.querySelectorAll('.spray-bubble'));
            
            const pushingHand = wrapper.querySelector('.pushing-hand') as HTMLElement;
            const sprayerHead = wrapper.querySelector('.sprayer-head') as HTMLElement;
            const gearsContainer = wrapper.querySelector('svg .gears') as HTMLElement;
            const gearConnector = wrapper.querySelector('.gear-connector') as HTMLElement;
            
            const pullSystemContainer = wrapper.querySelector('.pull-system') as HTMLElement;
            const checkboxPullLine = wrapper.querySelector('.checkbox-pull-line') as HTMLElement;
            const checkboxPullCircle = wrapper.querySelector('.checkbox-pull-circle') as HTMLElement;
            const btnPullLine = wrapper.querySelector('.submit-btn-connector') as HTMLElement;
            const btnHandlerCircle = wrapper.querySelector('.submit-btn-circle') as HTMLElement;
            
            const spiralContainer = wrapper.querySelector('.spiral-container') as HTMLElement;
            const weightBigContainer = wrapper.querySelector('.weight-big-container') as HTMLElement;
            
            const scalesContainer = wrapper.querySelector('.scales-container') as HTMLElement;
            const scalesLine = wrapper.querySelector('.scales-moving-line') as HTMLElement;
            const weightBig = wrapper.querySelector('.weight-big') as HTMLElement;
            const spiralPath = wrapper.querySelector('.spiral-path') as HTMLElement;
            
            const carContainer = wrapper.querySelector('.car-container') as HTMLElement;
            const car = wrapper.querySelector('.car') as HTMLElement;
            const carInclineWrapper = wrapper.querySelector('.car-container g') as HTMLElement;
            const timingChains = Array.from(wrapper.querySelectorAll('.timing-chain'));
            const reelsConnector = wrapper.querySelector('.reels-connector') as HTMLElement;
            const carWeightConnector = wrapper.querySelector('.car-weight-connector') as HTMLElement;
            
            const grabbingHand = wrapper.querySelectorAll('.grabbing-hand');
            const grabbingHandOpenFingers = Array.from(wrapper.querySelectorAll('.grabbing-hand-finger-open'));
            const grabbingHandClosedFingers = Array.from(wrapper.querySelectorAll('.grabbing-hand-finger-closed'));
                    ...
                    if (dIdx === (data.length - 1)) {
                        tl.eventCallback("onUpdate", () => {
                            ...
                            if (deltaY > 8) {
                                ...
                                gsap.set(submitBtn, { color: "rgba(0, 0, 0, " + state.sumbitBtnTextOpacity + ")" });
            ...
            function createPullingTimeline(isFixed: boolean, BtnPulled: boolean) {
                ...
                function animatePullingLine() {
                    ...
                    const deg = (gsap.getProperty(submitBtn, "rotation") as number - 4) * Math.PI / 180;
            ...
            nameEl.addEventListener('input', () => {
                ...
                if (nameValid) {
                    ...
                    gearsTls.forEach(tl => {
                        if (!tl.paused()) {
                            gsap.to(tl, { timeScale: 0, onComplete: () => { tl.pause(); } });
                        }
                    });
            ...
            submitBtn.addEventListener('click', (e) => {
                ...
                if (emailValid && checkboxEl.checked && nameValid && sprayRepeatCounter > 1) {
                    gsap.to(wrapper.querySelectorAll("svg > *"), {
                        duration: .1, opacity: 0,
                        stagger: { each: 0.03, from: 'random', ease: 'none' }
                    });
                    gsap.to(wrapper.querySelectorAll(".form-row"), {
                        delay: .4, duration: .1, opacity: 0, stagger: .1
                    });
                }
            });
        };
